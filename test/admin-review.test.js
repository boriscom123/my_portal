...
test('автор видит обложку, расшифровку и файлы буфера', skipWithoutDb, async () => {
  await withTestDb(async (pool) => {
    ...
    await withServer(app, async (base) => {
      ...
      // Поле реплики многострочное: на телефоне длинная реплика в одну строку
      // уезжает за край экрана, и не видно, что там дальше.
      assert.match(html, /<textarea[^>]*data-segment="/);
...
test('во время сборки кнопка пересборки выключена и объяснена', skipWithoutDb, async () => {
  await withTestDb(async (pool) => {
    ...
    await withServer(app, async (base) => {
      ...
      assert.match(html, /disabled title="Идёт другая работа"/);
      // Выключенная кнопка без объяснения читается как поломка. И называть
      // происходящее «пересборкой» нельзя: идти может копирование с Диска,
      // которое мы намеренно отделили от пересборки.
      assert.match(html, /Сейчас идёт другая работа: обрабатывается: вырезаются паузы/);
    });
  });
});
...
test('ролики собираются по нажатию, а не сами', skipWithoutDb, async () => {
  await withTestDb(async (pool) => {
    ...
    await withServer(app, async (base) => {
      ...
      // в «Ролики», поле новой серии убрано — серии заводятся в настройках,
