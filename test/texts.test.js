...
test('запрос для картинки к новости — по-английски, одной сценой и без «no»', () => {
  // Модель рисования отрицаний не понимает: «no play buttons» рисует кнопку.
  // Поэтому просим сцену с предметами и только то, что есть на картинке.
...
test('теги урока ложатся в сцену обложки предметами, а не надписями', () => {
  // Заказчик 2026-09-15: теги урока должны по возможности попадать на
  // картинку — так обложка точнее про урок. Раньше они уходили «ключевыми
  // словами» без просьбы что-то с ними делать. Надписью их не просим: модели
  // рисуют буквы с ошибками.
  const prompt = buildCoverImagePrompt({ title: 'Портал на VPS', tags: ['docker', 'телеграм-бот'] });
  assert.match(prompt, /Теги урока: docker, телеграм-бот/);
  assert.match(prompt, /каждый тег[^.]*предмет/i);
  assert.match(prompt, /никаких надписей/);

  // Тегов нет — нет и пустой строки с ними и просьбы их показать.
  const bare = buildCoverImagePrompt({ title: 'Т', tags: [] });
  assert.doesNotMatch(bare, /Теги урока:/);
  assert.doesNotMatch(bare, /каждый тег/i);

  // В заготовке урока теги и запрос для обложки пишутся одним ответом —
  // обложку просят строить по только что выбранным тегам.
  assert.match(buildPrompt('Разбираем докер компоуз', ''), /coverPrompt[\s\S]*по тегам из пункта 3/);
});
