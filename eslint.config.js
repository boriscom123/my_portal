...
export default [
  ...
  js.configs.recommended,
  {
    ...
    rules: {
      ...
      eqeqeq: 'error',
      'no-console': 'off',
      // Имена — только латиницей. Правило записано в CLAUDE.md, но держалось
      // на внимательности, и кириллические имена уже дважды приходилось
      // вычищать вручную: один раз это молча сломало вход через Telegram.
      // Комментарии и тексты для человека остаются русскими — селектор
      // смотрит только на имена в синтаксисе.
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Identifier[name=/[\\u0400-\\u04FF]/]',
          message: 'Имя кириллицей. Имена — латиницей, русскими остаются комментарии и тексты.'
        }
      ]
    }
  },
