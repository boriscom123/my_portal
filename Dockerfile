...
COPY public ./public
# Один публичный корневой сертификат — им проверяется соединение с MAX, и
# больше ничем: чем именно и почему, написано в certs/README.md.
COPY certs ./certs
...
# font-dejavu нужен вшиванию субтитров в вертикальные ролики: libass берёт
# шрифты через fontconfig, а в образе их нет ни одного. Без шрифта ffmpeg
# завершается успешно и молча отдаёт ролик без единой подписи — проверено.
# DejaVu выбран за кириллицу и малый вес: Noto тянет за собой десятки мегабайт.
RUN apk add --no-cache ffmpeg libstdc++ font-dejavu fontconfig
