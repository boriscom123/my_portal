...
### Задача 8: Тонкий слой распознавания и адаптер Яндекса — ОТМЕНЕНА, см. отступления
...
### Задача 9: Шаг расшифровки — СДАНО 2026-09-03
...
### Задача 11: Тексты от модели — ОТМЕНЕНА, см. отступления
...
### Задача 13: Поиск по урокам — СДАНО 2026-09-04
...
### Задача 14: Вертикальные нарезки со вшитыми субтитрами — СДАНО 2026-09-04
...
### Задача 17: Ход конвейера в кабинете — СДАНО 2026-09-04
...
**Расшифровка считается на сервере, адаптер Яндекса не написан (задачи 8, 9).**
Заказчик отказался от Яндекс Облака: «только Яндекс диск или google drive».
Вместо адаптера к облаку — whisper.cpp в образе воркера и модель `small` в
квантованном виде. Опыт на этой машине дал двукратную скорость относительно
реального времени и полтора гигабайта памяти на пике; часовой урок считается
полчаса. Тонкий слой из задачи 8 остался, но за ним один поставщик:
`createSpeech(config)` отдаёт `null`, когда бинарника или модели нет, и портал
при этом работает — витрина и отзывы от расшифровки не зависят.

Модель качается в том при старте воркера, а не кладётся в образ: 182 МБ в
каждой сборке — это минуты на сборщике GitHub при каждом коммите и лишний
трафик у того, кто раздаёт модель бесплатно. Сборка образа выросла на три
минуты — столько занимает статическая сборка whisper.cpp.

**Заготовки из титров на тишине.** Первый же прогон на беззвучном файле выдал
«Субтитры субтитров Н.Новикова» — известное поведение модели, обученной на
видео с субтитрами: на участке без речи она уверенно печатает самую частую
строку из титров. Попади такое в субтитры урока, автор получил бы чужую
подпись на своём видео. Отсев вынесен в `isHallucination` со списком известных
заготовок и закреплён тестом на настоящем выводе того прогона.

**Запись без речи не роняет конвейер.** Урок может целиком показывать экран
под музыку. Субтитры делать не из чего, поэтому шаг расшифровки ставит сразу
обложку, а не субтитры: иначе урок застревал бы на полпути с ошибкой «нет
расшифровки». Проверено на настоящем файле заказчика — вся цепочка от
извлечения звука до состояния «ждёт проверки» проходит без ошибок.

**Ход конвейера показывает шаг, а не только состояние (задача 17).** В плане
на карточке значилось «обрабатывается». На настоящем уроке этот шаг длится
двадцать пять минут, и надпись всё это время не отвечает на единственный
вопрос автора — идёт ли вообще что-нибудь. Заказчик уже жаловался на это
дважды. Поэтому воркер записывает идущий шаг в `lessons.pipeline_job` при его
начале, а кабинет пишет «обрабатывается: распознаётся речь». Пока что-то
считается, страница кабинета перечитывается сама раз в двадцать секунд; форм
на ней нет, поэтому обновление ничего не сотрёт.

**Поиск живёт в src/services/search.js, а не в маршруте (задача 13).** План
клал `searchSegments` в `src/routes/search.js`, но тогда страница из
`src/routes/pages.js` импортировала бы маршрут ради одной функции. Весь SQL в
проекте живёт в `src/services/`, и поиск не исключение.

**Ссылка находки ведёт на урок, а не на секунду.** В плане было
`/lesson/<slug>#t=<секунда>`. Своего плеера у портала нет — зритель смотрит на
площадке, — и якорь сейчас никто не читает. Он оставлен на будущее, но время
находки написано словами: «на 12:05». Иначе ссылка обещала бы переход, которого
не происходит.

**Нарезки выбираются по плотности речи, а не по главам (задача 14).** План брал
границы фрагментов из `generated.chapters` — глав от модели, которых теперь не
будет вместе с облаком. Резать по таймеру нельзя: нарезка через каждые десять
минут обрывает мысль на полуслове. Поэтому урок делится на трети, и в каждой
берётся окно в сорок пять секунд с наибольшим числом сказанных знаков — там,
где автор говорит без пауз, он обычно и объясняет суть. Считаем по знакам, а
не по числу реплик: короткие «ага» и «вот» дали бы плотность там, где на деле
пауза.

**Субтитры фрагмента подрезаются и сдвигаются к нулю.** В плане в фильтр
уходил общий файл субтитров урока. Перемотка до `-i` обнуляет времена, и на
десятой минуте урока подписи опережали бы речь на десять минут. Под каждый
фрагмент пишется свой временный srt, который удаляется сразу после счёта.

**Три фрагмента вместо пяти.** На двух ядрах каждый — минуты работы, а
отсматривает их автор всё равно по одному.

**Тяжёлые шаги сами не повторяются.** Общая настройка очереди — три попытки с
нарастающей паузой. Для скачивания с Диска это верно: причина отказа там
временная. Для расшифровки нет: она идёт двадцать пять минут и падает по
причинам постоянным — нет файла, нет модели, битый звук. Три попытки означали
бы семьдесят пять минут двухъядерной машины на повторение одного и того же
отказа, и всё это время автор видел бы «обрабатывается». Решение о повторе
тяжёлого шага оставлено человеку: в кабинете для этого есть кнопка. Правило
живёт в одной функции `jobOptions`, потому что шаги ставят друг друга из семи
разных мест.

**Отсечение тишины перед распознаванием.** На настоящем уроке модель сорвалась
на тринадцатой минуте и до конца записи печатала одну строку из чужих титров:
895 повторов из 1077 реплик.

Сперва я решил, что в записи пропал звук, и сказал это заказчику. Он проверил
сам и возразил: на 21:49 звук есть. Он был прав. Ошибка была в методе — три
точечных замера уровня по двадцать секунд, и все три пришлись на паузы: автор
работает молча помногу. Сплошной проход показал речь по всей длине и тридцать
семь пауз длиннее десяти секунд.

Причина петли — сами паузы: на них модель додумывает текст и залипает в нём.
Отключение переноса контекста между окнами (`-mc 0`) не помогло, проверено на
куске с двенадцатой по двадцатую минуту. Помогло единственное: не давать
модели тишину. Отсечение (silero, меньше мегабайта, качается в тот же том)
убрало повторы полностью, ускорило счёт вдвое — молчание больше не считается —
и заметно подняло качество: «вертольную машину» стало «виртуальную машину».

Фильтры повторов и титров оставлены второй сетью: они дешёвые, а первая может
однажды не сработать.

**Урок из этого.** Проверять запись точечными замерами нельзя — в уроке с
паузами это гадание. Замер должен быть сплошным, и лучше довериться человеку,
который эту запись слышал.
