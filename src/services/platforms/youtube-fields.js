...
// Какую запись автор может выбрать для выкладки: смонтированную или полную.
export const VIDEO_VERSIONS = ['trimmed', 'source'];

/**
 * Запись для выкладки по выбору автора.
 * Выбора нет — как раньше, монтаж, а без него исходник. Выбранной записи нет —
 * null, а не другая: автор ждёт на площадке именно ту, что выбрал.
 * Вызывается из src/routes/admin.js.
 */
export function pickPublishVideo(assets, version = null) {
  if (!version) return pickVideoAsset(assets);
  return assets.find((asset) => asset.kind === version) ?? null;
}

/** Почему выбранной записи нет — словами для автора. */
export function missingVersionMessage(version) {
  return version === 'trimmed'
    ? 'Смонтированной записи нет — вырежьте паузы или выберите полную запись'
    : 'Полной записи нет в буфере — вероятно, она удалена по сроку; выберите смонтированную или загрузите запись заново';
}
