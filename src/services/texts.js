...
${timeline ? 5 : 4}. coverPrompt — запрос для рисовальщика обложки урока, по-английски, одна
   связная фраза 40–70 слов: одна конкретная сцена из мира IT и искусственного
   интеллекта — серверы, чипы, потоки данных, узлы нейросети, облачная
   инфраструктура, рабочее место разработчика, — которая через метафору
   показывает суть урока. Строй сцену по тегам из пункта 3: каждый, какой
   получится, покажи узнаваемым предметом, а не надписью. Пиши только то, что есть на картинке: рисовальщик
   ...
   интерфейса, надписи и людей. Цвета — под сюжет, плоская векторная графика.
...
— одна конкретная сцена из мира IT и искусственного интеллекта — серверы,
  чипы и платы, потоки данных, узлы нейросети, облачная инфраструктура,
  рабочее место разработчика, — которая через метафору показывает суть заметки;
...
— стиль: современная технологическая иллюстрация для IT-портала; цвета подбери
  под сюжет — своей палитры у картинки нет;
...
${tags.length ? `Теги урока: ${tags.join(', ')}\n` : ''}
Составь ОДИН запрос на английском языке, по которому рисовальщик сделает
обложку этого урока.
...
— одна конкретная сцена из мира IT и искусственного интеллекта — серверы,
  чипы и платы, потоки данных, узлы нейросети, облачная инфраструктура,
  рабочее место разработчика, — которая через метафору показывает суть темы;
  абстракцию вроде «данные превращаются в каналы» модель рисует значком;${
    // Теги точнее заголовка говорят, про что урок: заказчик 2026-09-15
    // попросил, чтобы они попадали на обложку. Предметами, а не словами —
    // надписи модели рисуют с ошибками.
    tags.length
      ? `
— каждый тег урока по возможности покажи узнаваемым предметом в этой сцене:
  docker — контейнеры, база данных — цилиндры хранилища, телеграм-бот —
  бумажный самолётик с сообщениями; тег, которому не найти предмета, пропусти,
  чтобы сцена не развалилась на россыпь значков;`
      : ''
  }
...
— цвета подбери под сюжет — своей палитры у обложки нет;
