...
export const JOBS = {
  ...
  // Каналы: анонс урока с обложкой и ссылками. Имя своё у каждого — запуск
...
const NO_RETRY_JOBS = new Set([
  ...
  JOBS.publishYoutube,
  // Анонсы в каналы по той же причине: повтор после неясного отказа означает
  ...
  // Рисование обложки: слой рисования сам перебирает модели на «занято», а
  // повтор всей задачи через полминуты снимал бы отметку «рисуется» на первой
